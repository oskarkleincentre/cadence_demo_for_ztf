...
if __name__=='__main__':
    ...
    df = read_scheduler(scheduler_fname).ix[30000:]
    dfs = np.array_split(df.head(numRows), numSplits)
    def make_images(i):
        ztf_data = '/home/rbisw/data/ZTF'
...
#        vf = Pointings(ztf_fields, offset=0.)
        # fig, ax, m = vf.generate_image(1, df, snsims=simsdf)
        # fig.savefig('test.png')
        
#        vf.generate_images(dfs[i].index.values, dfs[i], snsims=simsdf, savefig=True,
#                           outdir=ztf_figs)
        tsplitend = time.time()
